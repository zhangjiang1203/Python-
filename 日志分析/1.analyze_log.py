...
file_path = os.path.join(workdir, "analyze_result.xls")
# 过滤的组件名
...
def analyze_log():
    with open("Analyze DYHeart_2024-01-16T13-34-12.txt", "r", encoding='utf-8') as f:
        # 查找四类错误
        # 未使用变量错误
        deadStoreError = []
        # 逻辑错误
        ...
        for line in f:
            if " warning: " in line:
                ...
                if (moduleName not in ignore_module) & ("DYTarget" not in fileName):
                    #获取错误信息类型
                    # matches = re.findall(r"\[(.*)\]", errorStr)
                    # for match in matches:
                    #     if match not in errorType:
                    #         errorType.append(match)
                    ...
                    if retainCycles in errorStr:
                        errorStr = re.sub(subPattern, "", errorStr)
                        retainCyclesError.append([moduleName, fileName, errorStr, "2", "","代码中可能存在一个保留环，可能导致内存泄露"])

                    if unreachableCode in errorStr:
                        errorStr = re.sub(subPattern, "", errorStr)
                        unreachableError.append([moduleName, fileName, errorStr, "3", "","代码因为程序逻辑而永远不会被执行"])

                    if uninitialized in errorStr:
                        errorStr = re.sub(subPattern, "", errorStr)
                        uninitializedError.append([moduleName, fileName, errorStr, "4", "","代码中存在一些变量在某些执行路径下可能未初始化就被使用了"])

                    if performSelectorLeaks in errorStr:
                        errorStr = re.sub(subPattern, "", errorStr)
                        performSelectorLeaksError.append([moduleName, fileName, errorStr, "5", "","使用 performSelector: 可能导致内存泄露的情况"])

                    if objCProperty in errorStr:
                        errorStr = re.sub(subPattern, "", errorStr)
                        propertyDecorateError.append([moduleName, fileName, errorStr, "6", "","属性修饰问题，结合上下文"])

                    if missingSuperCall in errorStr or missingSuperCalls in errorStr:
                        errorStr = re.sub(subPattern, "", errorStr)
                        missingSuperCallError.append([moduleName, fileName, errorStr, "7", "","子类中的对应方法中未正确调用父类的实现"])
