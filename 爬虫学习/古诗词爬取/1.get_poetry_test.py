...
def get_peotriers_with_dynasty(dynasty):
    ...
    for i in range(1,100):
        ...
        try:
            ...
            try:
                sql = """insert into Author(name,dynasty,author_desc,avator_img) values(?,?,?,?);"""
            ...
            for author in authors:
                # 获取当前作者的数据库索引
                sql = """select id from Author where name='%s';""" %author[0]
                cursor.execute(sql)
                result = cursor.fetchall()
                # result 是一个字典数组
                author_id = result[0][0]
                author_name = author[0]
                get_all_author_portry(author_name,author_id)
        except:
            print("get_peotriers_with_dynasty error %s" %orignal_url)
...
def get_all_author_info(url,dynasty):
    ...
    try:
        ...
        # print("诗人头像===%s" %avator)
        return (name[0], dynasty, desc[0], avator[0])
    except:
...
def get_poetry_content_info(url,author_id):
    ...
    # 插入数据
    try:
        sql = 'insert into Poetry(title,content,comment,author_id) values(?,?,?,?);'
